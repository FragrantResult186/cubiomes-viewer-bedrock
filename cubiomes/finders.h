...
static inline ATTR(const)
Pos getJFeaturePos(StructureConfig config, uint64_t seed, int regX, int regZ);

static inline ATTR(const)
Pos getJFeatureChunkInRegion(StructureConfig config, uint64_t seed, int regX, int regZ);

static inline ATTR(const)
Pos getJLargeStructurePos(StructureConfig config, uint64_t seed, int regX, int regZ);
...
Pos getFeaturePos(StructureConfig config, uint64_t seed, int regX, int regZ)
{
    ...
    pos.x = (int)(((uint64_t)regX*config.regionSize + pos.x) << 4)+8;//+8 offset
    pos.z = (int)(((uint64_t)regZ*config.regionSize + pos.z) << 4)+8;//+8 offset
    return pos;
}
...
Pos getJFeaturePos(StructureConfig config, uint64_t seed, int regX, int regZ)
{
    Pos pos = getJFeatureChunkInRegion(config, seed, regX, regZ);
...
Pos getJLargeStructurePos(StructureConfig config, uint64_t seed, int regX, int regZ)
{
    Pos pos = getJLargeStructureChunkInRegion(config, seed, regX, regZ);
